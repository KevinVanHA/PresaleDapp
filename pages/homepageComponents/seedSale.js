...
export default function SeedSale() {
        ...
        return (
          <div id="toast-simple" className="flex justify-center items-center p-4 space-x-4 w-full max-w-xs text-white bg-neutral-800 rounded-lg divide-x divide-gray-200 shadow space-x" role="alert">
            ...
            <div className="pl-4 text-sm font-normal">
              You own already {new Intl.NumberFormat().format(this.totalAmount)} Token<br />
              You're still on time to buy more!
            </div>
          </div>
        );
      } else {
  ...
  const { data: userVestingData } = useContractRead({
    ...
    args: useAccountAddress ? [useAccountAddress, process.env.NEXT_PUBLIC_PRESALE_ID] : undefined,
    watch: true,
  ...
  const { data: presaleData, error: presaleDataError, isError: presaleIsError, isLoading: presaleIsLoading, status: presaleStatus } = useContractRead({
    address: process.env.NEXT_PUBLIC_CONTRACT_ADDRESS?.toString(),
    abi: process.env.NEXT_PUBLIC_CONTRACT_ABI,
  ...
  useEffect(() => {
    if (!useAccountAddress) {
      ...
      setDisplayPresaleData("");
      const userVesting = new UserVesting(userVestingData);
